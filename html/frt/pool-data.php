...
// Choose a lock directory that is writable; fall back to /tmp if needed
$LOCK_DIR = $CACHE_DIR;
if (!is_dir($LOCK_DIR) || !is_writable($LOCK_DIR)) {
	$LOCK_DIR = sys_get_temp_dir() . '/frt_pool_locks';
	if (!file_exists($LOCK_DIR)) {
		@mkdir($LOCK_DIR, 0777, true);
	}
}

// If the configured cache directory exists but isn't writable by PHP, fall back to /tmp
if (is_dir($CACHE_DIR) && !is_writable($CACHE_DIR)) {
	$CACHE_DIR = sys_get_temp_dir() . '/frt_pool_cache';
...
// Expose request id and rpc settings via headers for debugging
header('X-Request-Id: ' . $REQUEST_ID);
header('X-RPC-Primary: ' . $RPC_URL);
header('X-RPC-Fallbacks: ' . (empty($FALLBACK_RPC_URLS) ? '' : implode(',', $FALLBACK_RPC_URLS)));

// Create cache directory if it doesn't exist
if (!file_exists($CACHE_DIR)) {
    if (!mkdir($CACHE_DIR, 0755, true)) {
        error_log("Failed to create cache directory: $CACHE_DIR");
        debugLog('cache_dir_create_failed', ['dir' => $CACHE_DIR]);
...
/**
 * Debug logging (line-delimited JSON)
 */
function debugLog($message, $context = []) {
	global $DEBUG_LOG_FILE, $REQUEST_ID;
...
function acquireLock($poolAddress, $timeout = 30) {
    ...
    while (time() - $startTime < $timeout) {
        ...
        if ($lockHandle && flock($lockHandle, LOCK_EX | LOCK_NB)) {
            ...
            debugLog('lock_busy', ['lockFile' => $lockFile]);
        }
 ...
 */
function saveToCache($poolAddress, $rpcResponse, $parsedPoolData = null) {
    ...
    if ($parsedPoolData !== null) {
        $cacheData['parsed_pool_data'] = $parsedPoolData;
    }
...
/**
 * Update existing cache file with parsed_pool_data (best-effort)
 */
function updateCacheWithParsedData($poolAddress, $parsedPoolData) {
    global $CACHE_DIR, $SCHEMA_VERSION;
    $filename = sanitizeFilename($poolAddress);
    $cacheFile = $CACHE_DIR . '/' . $filename . '.json';
    $tempFile = $cacheFile . '.tmp';
    ...
    if (!$cacheData || !isset($cacheData['schema_version']) || $cacheData['schema_version'] !== $SCHEMA_VERSION) {
        return false;
    }
    $cacheData['parsed_pool_data'] = $parsedPoolData;
    $newJson = json_encode($cacheData, JSON_PRETTY_PRINT);
    if ($newJson === false) {
        return false;
    }
...
debugLog('request_received', [
    'poolAddress_prefix' => substr($poolAddress, 0, 8) . '...',
...
if (!validatePoolAddress($poolAddress)) {
    ...
    debugLog('invalid_address', ['provided' => $poolAddress]);
    echo json_encode([
...
debugLog('refresh_check', ['shouldRefresh' => $shouldRefresh]);

// Try to load from cache first
$cachedData = loadFromCache($poolAddress);
debugLog('cache_checked', ['hit' => $cachedData !== false]);
...
if ($cachedData !== false && !$shouldRefresh) {
    ...
    if (!isset($cachedData['parsed_pool_data'])) {
        try {
            ...
            // Persist parsed data so subsequent hits skip parse/metadata
            updateCacheWithParsedData($poolAddress, $parsedPoolData);
    ...
    header('X-Generated-At: ' . $cachedData['generated_at']);
        ...
        debugLog('served_cache', ['status' => 'hit', 'generated_at' => $cachedData['generated_at'] ?? null]);
    exit;
}
...
if ($shouldRefresh || $cachedData === false) {
    ...
    debugLog('lock_attempt', ['acquired' => (bool)$lockHandle]);
    ...
    if (!$lockHandle) {
        ...
        if ($cachedData !== false) {
            ...
            echo json_encode($cachedData);
            debugLog('served_cache', ['status' => 'hit-lock-failed', 'generated_at' => $cachedData['generated_at'] ?? null]);
            exit;
        } else {
    ...
    if ($cachedData !== false && !needsRefresh($poolAddress, $refreshRequested)) {
        ...
        echo json_encode($cachedData);
        debugLog('served_cache', ['status' => 'hit-after-lock', 'generated_at' => $cachedData['generated_at'] ?? null]);
        exit;
    }
}
...
debugLog('fetching_from_rpc', ['reason' => $cachedData === false ? 'miss' : 'refresh']);

$rpcData = fetchPoolDataFromRPC($poolAddress);
debugLog('rpc_result', ['ok' => $rpcData !== false && $rpcData !== null]);
...
if ($rpcData === null) {
    ...
    debugLog('pool_not_found_or_invalid');
    exit;
}
...
try {
    ...
    debugLog('parsed_success');
} catch (Exception $e) {
    logMetrics($poolAddress, 'pool_parse_failed', null, null, $e->getMessage());
    debugLog('parsed_failed', ['error' => $e->getMessage()]);
    // Continue with raw data if parsing fails
}

// Save to cache including parsed data to speed future hits (best-effort)
saveToCache($poolAddress, $rpcData, $parsedPoolData);
debugLog('cache_saved');
...
debugLog('request_complete', ['ms' => $totalTime]);
